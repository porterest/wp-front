...
export const fetchCandlestickData = async (
    ...
    try {
        const response = await fetch(`${API_BASE_URL}?symbol=${symbol}&interval=${interval}&limit=${limit}`);
        if (!response.ok) {
...
export const getCandlestickData10m = async (symbol: string): Promise<AggregatedCandle[]> => {
