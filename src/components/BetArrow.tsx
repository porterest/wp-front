...
interface BetArrowProps {
    ...
    onShowConfirmButton: (show: boolean) => void; // Callback для управления кнопкой подтверждения
...
const BetArrow: React.FC<BetArrowProps> = ({
    ...
    const { gl, camera, scene } = useThree(); // Контекст Three.js
    ...
    const handlePointerMove = (event: PointerEvent) => {
        ...
        const direction = new THREE.Vector3().subVectors(intersection, fixedPreviousBetEnd);
    ...
    useFrame(() => {
        ...
        const yellowLinePositions = [0, 0, 0, fixedPreviousBetEnd.x, fixedPreviousBetEnd.y, fixedPreviousBetEnd.z];
    ...
    useFrame(() => {
        ...
        const yellowLinePositions = [
            0, 0, 0,
            fixedPreviousBetEnd.x, fixedPreviousBetEnd.y, fixedPreviousBetEnd.z,
        ];
        yellowLine.current?.geometry.setPositions(yellowLinePositions);
    ...
    return (
        <>
            {/* Текст депозита */}
            <Text
                position={[fixedPreviousBetEnd.x, fixedPreviousBetEnd.y + 1, fixedPreviousBetEnd.z]}
                fontSize={0.3}
                color="lightgreen"
                anchorX="center"
                anchorY="middle"
            ...
            {/* Текст ставки */}
            <Text
                position={[xValue, yValue +0.5, dashedLineStart.z]}
                fontSize={0.3}
                color="white"
                anchorX="center"
                anchorY="middle"
