...
//
//
// //тут стрелка отлично удлиняется и укорачивается, состояния между осями сохраняются
...
const BetLines: React.FC<BetLinesProps> = ({
  ...
  // Фиксированное значение оси "времени"
  const fixedTimeValue = 3.5;
  ...
  const handlePointerMove = (e: PointerEvent) => {
    ...
    // Найдём точку пересечения луча с плоскостью x=fixedTimeValue
    // Конструируем плоскость x=3.5 (нормаль (1,0,0), расстояние=-3.5)
    const plane = new THREE.Plane(new THREE.Vector3(1, 0, 0), -fixedTimeValue);
    const intersectPt = new THREE.Vector3();
    if (!raycaster.current.ray.intersectPlane(plane, intersectPt)) {
      return;
    }
    ...
    // Частичное обновление с учётом axisMode
    const updatedPos = betPosition.clone(); // текущее положение

    // Если меняем X, возьмём x из intersectPt, а y,z оставим от betPosition
    if (axisMode === "X") {
      updatedPos.x = intersectPt.x; // но мы фиксируем x=3.5, поэтому если axisMode="X", возможно нужно убрать фикс?
      // Однако, если хотим дать возможность движению по X, убираем жёсткое x=3.5:
      // Но в задаче говорится о "фиксированной оси времени", значит движение по X не предполагается?
      // Если надо всё же разрешить X, закомментируйте строку выше. Ниже оставим как есть.
    } else if (axisMode === "Y") {
      updatedPos.y = intersectPt.y; // менять Y
    }
